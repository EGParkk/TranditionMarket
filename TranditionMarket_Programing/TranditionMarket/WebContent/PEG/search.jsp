<%@page import="PEG.data.MarketImgBean"%>
<%@page import="java.util.List"%>
<%@page import="org.apache.jasper.tagplugins.jstl.core.ForEach"%>
<%@page import="java.util.ArrayList"%>
<%@page import="PEG.data.MarketBean"%>
	...
	<form action="<%=request.getContextPath()%>/Search" method="get">
		<div class="input-group input-group-lg">
			<span class="input-group-text" id="inputGroup-sizing-lg"> <i
				...
				placeholder="<%=request.getAttribute("searchKW")%>" name="searchKW" />
		</div>
	</form>
		...
		<div class="row featurette">
			<div class="col-md-7 order-md-2">
				<h2 class="featurette-heading">
					<button type="submit" name="mno" value="<%=searchedMK.get(i).getMno() %>" class="nav-link bg-dark px-2 text-bule border border-0"><%=searchedMK.get(i).getMname()%></button>
				</h2>
				<p class="lead">
			...
			<div class="col-md-5 order-md-1">
				<div class="bd-placeholder-img bd-placeholder-img-lg featurette-image img-fluid mx-auto">
					...
					if(!MKIMGdataNull.get(i)){
					%>
              			<button type="submit" class="bg-dark border border-0" name="mno" value="<%=searchedMK.get(i).getMno() %>"><img style="width: 300px; height: 300px; " alt="<%=searchedMK.get(i).getMname()%>" src="<%=searchedMKIMG.get(0).getUrl()%>"></button>
              		<%
					}else{
					%>
